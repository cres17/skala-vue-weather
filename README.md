...
- Vue Router의 동적 경로 문법과 Catch-all 문법이 현재 버전에서 어떻게 달라졌는지 확인하는데 도움을 받았습니다.
- `find()`가 도시를 찾지 못했을 때 화면에서 어떤 예외 처리가 필요한지 확인하는데 도움을 받았습니다.
- 구현 후 lint, production build, 브라우저 경로 전환을 검사할 때 요구사항이 빠진 부분이 없는지 체크하는데 도움을 받았습니다.
