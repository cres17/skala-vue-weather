을 만든 실습입니다. 상단 메뉴의 버튼을 누르면 날씨 목록과 도시 상세 페이지가 함께 섭씨(°C)와 화씨(°F)로 바뀝니다.
...
처음에는 “이 화면에서 쓰니까 이 컴포넌트에 두면 되겠지”라고 생각했습니다. 하지만 다른 페이지가 생기는 순간 같은 단위 값이나 즐겨찾기 목록을 복사해야 했고, 한쪽만 바뀌는 문제가 생겼습니다. 이번 과제를 거치며 **한 화면에서만 의미가 있으면 컴포넌트 상태, 화면을 넘어 공유되면 store 상태**라는 기준을 세울 수 있었습니다.
