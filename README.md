...
처음에는 검색할 때마다 `weatherList` 자체를 바꾸는 방향으로 생각했습니다. 하지만 이렇게 하면 검색어를 지웠을 때 원래 목록을 다시 만들거나 따로 저장해야 해서 상태가 꼬일 수 있었습니다. 공백만 입력했을 때를 어떻게 처리할지도 애매했습니다. AI에게 “원본 목록은 그대로 두고 검색 결과만 바꾸고 싶다”고 질문했고, `searchQuery`와 `weatherList`를 바꾸는 대신 결과를 `computed`로 계산하는 방식을 적용했습니다. `trim()`으로 공백을 제거한 값을 먼저 만들고 값이 없으면 전체 목록을 반환하도록 했습니다.
...
### 2. 즐겨찾기·상세보기 버튼을 눌러도 카드 선택이 함께 실행되는 문제
 카드 전체에 `@click="selectCity(city)"`를 걸어 둔 상태에서 별 또는 상세보기 버튼을 눌렀더니 버튼 기능이 실행된 뒤 카드 선택 기능까지 같이 실행됐습니다. 버튼만 눌렀는데 상태바 문구까지 바뀌어서 처음에는 왜 두 함수가 실행되는지 알기 어려웠습니다. AI 도움으로 자식 요소의 클릭 이벤트가 부모 요소까지 전달되는 이벤트 버블링 때문이라는 것을 알았습니다. 버튼은 카드 안에 있으므로 버튼에서 발생한 클릭이 카드의 `@click`까지 전달되고 있었습니다. 버튼 클릭에는 Vue 이벤트 수식어인 `.stop`을 붙여 부모 카드로 이벤트가 전달되지 않게 했습니다.
