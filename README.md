...
이번 과제에서 가장 신경 쓴 부분은 “페이지를 나눴지만 데이터가 끊기지 않게 만드는 것”이었습니다. 홈에서는 서울 카드가 보이는데 상세 페이지에서는 서울 정보를 못 찾는 식의 문제가 생기지 않도록, 날씨 Mock Data를 한 파일에 모으고 홈과 상세 페이지가 같이 사용하도록 구성했습니다.
