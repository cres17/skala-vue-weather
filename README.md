...
- 대기질 측정 등급이 API 응답에 없으면 PM10·PM2.5 수치로 등급을 보완 계산합니다.
- UTCI와 대기질 카드는 위험도에 따라 파랑·민트·노랑·빨강 계열로 달라집니다.
...
AirKorea 실시간 응답은 측정소와 갱신 시점에 따라 통합대기환경지수 또는 등급 필드가 비어 있을 수 있습니다. 이 경우 카드 전체를 비우지 않고 PM10·PM2.5 농도를 기준으로 `좋음·보통·나쁨·매우 나쁨` 등급을 계산해 표시하도록 보완했습니다. 대기질 조회는 가까운 AirKorea 측정소를 기준으로 하므로, 선택한 행정구역의 평균값과는 다를 수 있습니다.

### 연돌기준정보 API를 실시간 대기질에 사용하지 않은 이유

`AirqualityService/getStackStdr`는 사업장 코드와 연돌명을 입력받아 특정 시설의 설계기준·배출량을 조회하는 API입니다. 도시별 실시간 PM10·PM2.5 또는 통합대기환경지수를 제공하지 않으므로, 현재 대기질 카드에는 AirKorea 측정소 실시간 API를 사용했습니다.

## 환경변수

```env
KMA_API_KEY=기상청_API_인증키
AIR_KOREA_API_KEY=공공데이터포털_AirKorea_서비스키
```

두 키 모두 서버에서만 읽으며, Vercel 배포 환경에도 같은 이름으로 등록해야 합니다.
