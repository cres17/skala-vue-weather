...
<template>
  ...
  >
    ...
    <!-- 내부 버튼 클릭이 카드 선택 이벤트까지 올라가지 않도록 stop을 사용한다. -->
    <button
...
.weather-card {
  ...
  outline: none;
  background: #fff;
  box-shadow: 0 8px 25px rgb(56 92 108 / 5%);
  transition:
    transform 0.2s,
    box-shadow 0.2s,
    border-color 0.2s;
...
.weather-card--selected {
  border-color: #72bcc8;
  box-shadow: 0 14px 28px rgb(28 98 119 / 13%);
  transform: translateY(-4px);
}

.weather-card--selected {
  background: linear-gradient(155deg, #fff 55%, #eefafa);
}

.weather-card__topline,
.weather-card__main,
.weather-card__meta div,
...
.weather-card__icon {
  font-size: 3.5rem;
  filter: drop-shadow(0 5px 4px rgb(94 152 168 / 13%));
}

.weather-card__main strong {
  color: var(--text-color);
  font-size: 2rem;
  letter-spacing: -0.07em;
}

.weather-card__main p {
  margin: 4px 0 0;
  color: #5c9ba5;
  font-weight: 600;
}
...
.weather-card__meta dt {
  color: #8b9da5;
  font-size: 0.78rem;
}

.weather-card__meta dd {
  margin: 0;
  color: #52717e;
  font-size: 0.78rem;
  font-weight: 700;
